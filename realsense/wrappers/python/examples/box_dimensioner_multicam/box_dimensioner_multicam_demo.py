...
# 부피 데이터를 전송하는 함수
def send_volume_to_datahub(total_volume_l):
    edgeData = EdgeData()
...
# 웹캠 캡처 및 텍스트 인식 함수
def capture_and_recognize(saved_words):
...
def run_demo():
    ...
    while True:
        ...
        if len(clusters) == 1:
            ...
            visualise_measurements(frames_devices, bounding_box_points_color_image, length, width, height)      
        time.sleep(0.5)
    capture_and_recognize(["pepsi", "25", "04", "02", "24", "09", "18"])
