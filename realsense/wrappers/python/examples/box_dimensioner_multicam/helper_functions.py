...
def get_clipped_pointcloud(pointcloud, boundary, chessboard_height=0.005, z_threshold=2.0):
   """
   Get the clipped pointcloud within the X and Y bounds specified in the boundary
   ...
   #pointcloud = pointcloud[:, pointcloud[2, :] >= -z_threshold]
    #pointcloud = pointcloud[:, pointcloud[2, :] > chessboard_height + z_threshold]
   #if pointcloud.size > 0:
   #   max_z = np.max(pointcloud[2, :])
   #   pointcloud = pointcloud[:, pointcloud[2, :] < max_z + z_threshold]
